...
Deno.serve(async (r) => {
  ...
  try {
    ...
    if (action === "capture") {
      ...
      try {
        const internalKey = JSON.parse(Deno.env.get("SUPABASE_SECRET_KEYS") || "{}").default || Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || "";
        const emailResponse = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/access-code-issue`, {
          method: "POST",
          headers: { Authorization: "Bearer " + internalKey, "Content-Type": "application/json" },
          body: JSON.stringify({ paymentId: pay.id }),
        });
        if (!emailResponse.ok) console.error("paypal access-code trigger failed", emailResponse.status);
      } catch (emailError) {
        console.error("paypal access-code trigger error", emailError);
      }
