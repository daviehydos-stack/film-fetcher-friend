...
function evaluateConnection(): NetworkStatus {
  ...
  if(saveData||effectiveType.includes("2g")||downlink<1)return{tier:"slow",saveData,effectiveType,downlink,isFast:false,online:navigator.onLine};
  if(effectiveType==="3g"||downlink<2.5)return{tier:"moderate",saveData,effectiveType,downlink,isFast:false,online:navigator.onLine};
  if(downlink>=6)return{tier:"ultra",saveData,effectiveType,downlink,isFast:true,online:navigator.onLine};
