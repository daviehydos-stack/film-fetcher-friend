...
export type ReturnContext={path:string;scrollY:number;label?:string;createdAt:number};
export function rememberReturnContext(label?:string){
 if(typeof window==="undefined")return;
 const base=(import.meta.env.BASE_URL||"/").replace(/\/$/,"");
 let pathname=window.location.pathname;
 if(base&&base!=="/"&&pathname.startsWith(base))pathname=pathname.slice(base.length)||"/";
 sessionStorage.setItem(KEY,JSON.stringify({path:pathname+window.location.search+window.location.hash,scrollY:window.scrollY,label,createdAt:Date.now()}));
}
