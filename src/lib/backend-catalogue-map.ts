...
export const KNOWN_PRODUCTS:Record<string,KnownProduct>={
  ...
  [BACKEND_PRODUCT_IDS.masterclass]:{id:BACKEND_PRODUCT_IDS.masterclass,slug:"jennifer-gatero-writing-masterclass",name:"Jennifer Gatero Writing Masterclass",titleName:"Jennifer Gatero Writing Masterclass",priceMinor:500,currency:"KES",artwork:"https://static.wixstatic.com/media/57086b_3c2c8674850045c78312f3397ae32e13~mv2.jpg/v1/fill/w_588,h_416,al_c,q_85,enc_auto/file.jpeg"},
};
export function knownProduct(key:string){const raw=String(key||"").replace(/^title\//,"").replace(/^\/+|\/+$/g,"");if(KNOWN_PRODUCTS[raw])return KNOWN_PRODUCTS[raw];const bySlug=Object.values(KNOWN_PRODUCTS).find(p=>p.slug===raw);if(bySlug)return bySlug;const legacyId=productForLegacyContent(raw);return KNOWN_PRODUCTS[legacyId]||null}
