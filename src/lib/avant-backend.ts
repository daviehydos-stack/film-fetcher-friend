...
export const loginWithAccessCode=async(accessCode:string,deviceId:string,deviceName?:string,expectedProductId?:string)=>{const cleanCode=accessCode.trim().toUpperCase();const result=await request<any>("access-code-login",null,{method:"POST",body:JSON.stringify({accessCode:cleanCode,deviceId,deviceName,...(expectedProductId?{productId:expectedProductId}:{})})});const assignedProduct=result?.payment?.product_id||result?.productId;if(expectedProductId&&assignedProduct&&assignedProduct!==expectedProductId)throw new Error("This access code belongs to a different title or season.");if(expectedProductId)recordVerifiedPurchase({productId:expectedProductId,reference:result?.payment?.reference,accessCode:cleanCode});adoptGuestCustomer(result?.externalCustomerId);clearPlaybackCache();broadcastAccessChanged({source:"access-code",productId:expectedProductId});return result};
...
export function recordVerifiedPurchase(entry:{productId?:string|null;reference?:string|null;accessCode?:string|null}){if(typeof window==="undefined")return;const productId=String(entry.productId||"").trim();if(!productId)return;const list=readVerifiedPurchases().filter(x=>x.productId!==productId);const code=String(entry.accessCode||"").trim().toUpperCase();list.push({productId,reference:String(entry.reference||""),accessCode:code||undefined,at:Date.now()});try{localStorage.setItem(PURCHASE_KEY,JSON.stringify(list.slice(-50)));if(code)localStorage.setItem(`avant_code_${productId}`,code)}catch{}}
...
export function getProductAccessCode(productId?:string|null){if(typeof window==="undefined"||!productId)return"";try{return localStorage.getItem(`avant_code_${productId}`)||readVerifiedPurchases().find(x=>x.productId===productId)?.accessCode||""}catch{return""}}
