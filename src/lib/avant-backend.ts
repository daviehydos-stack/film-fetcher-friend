...
export const issueAccessCode=(token:string|null,paymentId:string)=>request<any>("access-code-issue",token,{method:"POST",body:JSON.stringify({paymentId})});
export const loginWithAccessCode=async(accessCode:string,deviceId:string,deviceName?:string)=>{const result=await request<any>("access-code-login",null,{method:"POST",body:JSON.stringify({accessCode,deviceId,deviceName})});adoptGuestCustomer(result?.externalCustomerId);clearPlaybackCache();broadcastAccessChanged({source:"access-code"});return result};
