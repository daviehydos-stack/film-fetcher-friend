...
async function request<T>(name:string,token?:string|null,init:RequestInit={}){const adminProof=typeof window!=="undefined"&&name.startsWith("admin-")?sessionStorage.getItem("avant_admin_mfa_proof"):null;const headers={...(SUPABASE_ANON_KEY?{apikey:SUPABASE_ANON_KEY}:{}),...(token?{Authorization:`Bearer ${token}`} :{}),...(adminProof?{"x-admin-mfa":adminProof}:{}),"Content-Type":"application/json",...(init.headers||{})};const method=String(init.method||"GET").toUpperCase();const safeRetry=method==="GET"||method==="HEAD"||name==="payment-status"||name==="account-access"||name==="my-library";let last:any;for(let attempt=0;attempt<(safeRetry?2:1);attempt++){const controller=new AbortController();const timer=typeof window!=="undefined"?window.setTimeout(()=>controller.abort(),12000):null;try{const r=await fetch(`${SUPABASE_URL}/functions/v1/${name}`,{...init,signal:init.signal||controller.signal,headers});const body=await r.json().catch(()=>({}));if(!r.ok){const e:any=new Error(body.error||body.detail||`${name} failed (${r.status})`);e.status=r.status;e.body=body;throw e}return body as T}catch(e:any){last=e;if(e?.status||!safeRetry||attempt===1)break;if(typeof window!=="undefined")await new Promise(x=>window.setTimeout(x,900))}finally{if(timer)window.clearTimeout(timer)}}if(last?.name==="AbortError")throw new Error("Avant services took too long to respond. Please retry.");throw last||new Error("Unable to reach Avant services. Check your connection and retry.")}
let publicCatalogueCache:any=null,publicCataloguePending:Promise<any>|null=null,publicCatalogueAt=0;const PUBLIC_CATALOGUE_TTL=900000;const PUBLIC_CATALOGUE_STORAGE="avant_public_catalogue_v2";function hydratePublicCatalogue(){if(publicCatalogueCache||typeof window==="undefined")return;try{const raw=sessionStorage.getItem(PUBLIC_CATALOGUE_STORAGE);if(!raw)return;const saved=JSON.parse(raw);if(saved?.payload&&Date.now()-Number(saved.savedAt||0)<PUBLIC_CATALOGUE_TTL){publicCatalogueCache=saved.payload;publicCatalogueAt=Number(saved.savedAt)}}catch{}}const missingCatalogueKeys=new Set<string>();export const publicCatalogue=()=>{hydratePublicCatalogue();const now=Date.now();if(publicCatalogueCache&&now-publicCatalogueAt<PUBLIC_CATALOGUE_TTL)return Promise.resolve(publicCatalogueCache);if(publicCataloguePending)return publicCataloguePending;publicCataloguePending=request<any>("catalogue-public").then(x=>{publicCatalogueCache=x;publicCatalogueAt=Date.now();if(typeof window!=="undefined")try{sessionStorage.setItem(PUBLIC_CATALOGUE_STORAGE,JSON.stringify({savedAt:publicCatalogueAt,payload:x}))}catch{}return x}).catch(error=>{if(publicCatalogueCache)return publicCatalogueCache;throw error}).finally(()=>{publicCataloguePending=null});return publicCataloguePending};
export const resolveCatalogueKey=async(key:string)=>{if(missingCatalogueKeys.has(key))return null;
...
export const issueAccessCode=(token:string,paymentId:string)=>request<any>("access-code-issue",token,{method:"POST",body:JSON.stringify({paymentId})});
