...
async function request<T>(name:string,token?:string|null,init:RequestInit={}){const controller=new AbortController();const timer=typeof window!=="undefined"?window.setTimeout(()=>controller.abort(),20000):null;const adminProof=typeof window!=="undefined"&&name.startsWith("admin-")?sessionStorage.getItem("avant_admin_mfa_proof"):null;let r:Response;try{r=await fetch(`${SUPABASE_URL}/functions/v1/${name}`,{...init,signal:init.signal||controller.signal,headers:{...(token?{Authorization:`Bearer ${token}`}:{}),...(adminProof?{"x-admin-mfa":adminProof}:{}),"Content-Type":"application/json",...(init.headers||{})}})}catch(e:any){if(e?.name==="AbortError")throw new Error("The server took too long to respond. Please retry.");throw new Error("Unable to reach Avant services. Check your connection and retry.")}finally{if(timer)window.clearTimeout(timer)}const body=await r.json().catch(()=>({}));if(!r.ok){const e:any=new Error(body.error||`${name} failed (${r.status})`);e.status=r.status;e.body=body;throw e}return body as T}
