...
export function broadcastAccessChanged(detail:Record<string,unknown>={}){clearPlaybackCache();if(typeof window==="undefined")return;window.dispatchEvent(new CustomEvent(ACCESS_CHANGED_EVENT,{detail}));try{localStorage.setItem("avant_access_changed",JSON.stringify({at:Date.now(),...detail}))}catch{}}
