...
export function vimeoEmbedUrl(id:string,options:EmbedOptions={}){const p=new URLSearchParams({playsinline:"1",autoplay:options.autoplay?"1":"0",muted:options.muted?"1":"0",controls:options.controls===false?"0":"1",autopause:"0",dnt:"1",quality:"auto",responsive:"1",api:"1"});if(options.loop)p.set("loop","1");if(options.start&&options.start>0)p.set("t",String(Math.floor(options.start))+"s");return `https://player.vimeo.com/video/${encodeURIComponent(id)}?${p.toString()}`}
