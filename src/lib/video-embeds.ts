...
export function heroTrailerUrl(embedUrl:string,muted=true,controls=false){
 ...
 if(youtube){url.hostname="www.youtube.com";url.searchParams.set("mute",muted?"1":"0");url.searchParams.set("controls",controls?"1":"0");url.searchParams.set("playsinline","1");url.searchParams.set("rel","0");url.searchParams.set("enablejsapi","1");if(typeof window!=="undefined")url.searchParams.set("origin",window.location.origin);url.searchParams.set("loop","1");const id=url.pathname.split("/").filter(Boolean).pop();if(id)url.searchParams.set("playlist",id)}
