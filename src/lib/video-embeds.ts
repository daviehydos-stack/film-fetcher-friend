...
export function youtubeEmbedUrl(id:string,options:EmbedOptions={}){
 // YouTube is a transport/source only. Avant owns the visible playback controls.
 const params=new URLSearchParams({rel:"0",playsinline:"1",iv_load_policy:"3",disablekb:"1",fs:"0",controls:"0",enablejsapi:"1"});
 if(options.autoplay)params.set("autoplay","1"); if(options.start&&options.start>0)params.set("start",String(Math.floor(options.start))); if(options.end&&options.end>0)params.set("end",String(Math.floor(options.end))); if(typeof window!=="undefined")params.set("origin",window.location.origin); if(options.muted)params.set("mute","1"); if(options.loop){params.set("loop","1");params.set("playlist",id)}
