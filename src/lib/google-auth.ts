...
function validToken(){if(typeof window==="undefined")return null;const token=sessionStorage.getItem(TOKEN_KEY);if(!token)return null;const p=payload(token);if(!p?.sub||p.aud!==INTERNAL_GOOGLE_CLIENT_ID||Number(p.exp||0)*1000<=Date.now()+30000){sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem("avant_subscriber");return null}return token}
...
export async function signOutCustomer(){if(typeof window!=="undefined"){sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem("avant_subscriber");emitSession();window.dispatchEvent(new CustomEvent("avant:access-changed",{detail:{source:"sign-out"}}));try{await loadGoogle();window.google.accounts.id.disableAutoSelect()}catch{}}}
