...
async function googleCredential(loginHint?:string){if(signInFlight)return signInFlight;signInFlight=(async()=>{await loadGoogle();return new Promise<string>((resolve,reject)=>{let settled=false;const done=(token?:string,error?:Error)=>{if(settled)return;settled=true;if(token)resolve(token);else reject(error||new Error("Google Sign-In could not complete."))};window.google.accounts.id.initialize({client_id:INTERNAL_GOOGLE_CLIENT_ID,ux_mode:"popup",context:"signin",itp_support:true,login_hint:loginHint||undefined,auto_select:Boolean(loginHint),cancel_on_tap_outside:false,callback:(r:any)=>{if(!r?.credential){done(undefined,new Error("Google did not return a credential."));return}localStorage.setItem(TOKEN_KEY,r.credential);rememberCredential(r.credential);emitSession();window.dispatchEvent(new CustomEvent("avant:access-changed",{detail:{source:"sign-in"}}));done(r.credential)}});window.google.accounts.id.prompt((n:any)=>{if(n?.isNotDisplayed?.()||n?.isSkippedMoment?.())done(undefined,new Error("Google could not sign this account in automatically."))})});})().finally(()=>{signInFlight=null});return signInFlight}
