...
export async function authorizeContent(
  ...
  const entitlements = await store.findValidEntitlements(
    input.customerId,
    input.contentId,
    input.seasonId,
  )
  ...
  const entitlement = entitlements.find((item) =>
    entitlementGrantsContent(item, { contentId: input.contentId, seasonId: input.seasonId }),
  )

  return entitlement
    ? { authorized: true, reason: 'granted', entitlement }
    : { authorized: false, reason: 'no_entitlement' }
}
