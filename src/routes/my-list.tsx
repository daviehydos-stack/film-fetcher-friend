...
function MyList() {
  ...
  const [customer,setCustomer]=useState<any>(()=>{if(typeof window==="undefined")return undefined;try{const t=localStorage.getItem("avant_google_id_token");if(!t)return null;const payload=t.split(".")[1];if(!payload)return null;const p=JSON.parse(atob(payload.replace(/-/g,"+").replace(/_/g,"/")));return ((p as any)?.exp||0)*1000>Date.now()?{email:(p as any).email,name:(p as any).name,photoURL:(p as any).picture}:null}catch{return null}});
