      ...
      <main>
        <section className="relative min-h-[68svh] overflow-hidden bg-background sm:min-h-[76vh] lg:mx-auto lg:mt-6 lg:max-w-[1180px] lg:rounded-t-2xl">
          ...
          <div className="relative z-10 flex min-h-[72svh] max-w-3xl flex-col justify-end px-5 pb-10 pt-28 sm:min-h-[72vh] sm:px-10 sm:pb-16 lg:px-14">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm font-semibold"><span className="text-primary">Avant Original</span><span className="text-white/45">•</span><span>{item.year ?? "Avant"}</span><span className="text-white/45">•</span><span>{item.quality ?? "HD"}</span>{item.maturityRating ? <><span className="text-white/45">•</span><span className="rounded border border-white/35 px-1.5 py-0.5 text-xs">{item.maturityRating}</span></> : null}<span className="text-white/45">•</span><span>{item.genres.join(" · ")}</span>{item.episodes ? <><span className="text-white/45">•</span><span>{seasons.length > 1 ? `${seasons.length} seasons` : "Season 1"} · {item.episodes.length} episodes</span></> : null}</div>
