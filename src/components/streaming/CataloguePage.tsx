...
export function CataloguePage({type,title,intro}:{type:ContentType;title:string;intro:string}){
 ...
 return <StreamingShell><main className="min-h-[80vh] pb-20 pt-20">
  ...
  {hero?<section ref={heroRef} className="relative mx-5 mt-7 aspect-[16/7] min-h-[25rem] overflow-hidden rounded-[1.6rem] border border-white/10 bg-black shadow-2xl sm:mx-10 lg:mx-14">
   <img src={hero.backdrop} alt={`${hero.title} featured ${type === "movie" ? "movie" : "series"} artwork`} fetchPriority="high" decoding="async" className="absolute inset-0 size-full object-cover"/>
   {trailer&&heroInView?<iframe src={trailer} title={`${hero.title} preview`} allow="autoplay; encrypted-media; picture-in-picture" className="pointer-events-none absolute left-1/2 top-1/2 h-[56.25vw] min-h-full w-[177.78vh] min-w-full -translate-x-1/2 -translate-y-1/2 border-0"/>:null}
