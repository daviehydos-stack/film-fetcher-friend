...
export function CataloguePage({ type, title, intro }: { type: ContentType; title: string; intro: string }) {
  ...
  const pool = catalogue.filter((item) => item.type === type);
    ...
    <StreamingShell>
      <main className="mx-auto min-h-[75vh] max-w-[1500px] px-5 pb-20 pt-28 sm:px-10 lg:px-14">
        ...
        <div className="mt-10 flex flex-col gap-4 border-b border-border pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex gap-2 overflow-x-auto pb-1">{genres.map((item) => <Button key={item} size="sm" variant={genre === item ? "default" : "secondary"} onClick={() => setGenre(item)}>{item}</Button>)}</div>
          <label className="relative block w-full sm:w-72"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search titles" className="h-10 bg-surface pl-9" /></label>
        </div>
