      ...
      <div className="relative z-10 flex min-h-[76svh] max-w-[1600px] items-end px-5 pb-[max(3.5rem,env(safe-area-inset-bottom))] pt-[max(7rem,env(safe-area-inset-top))] sm:min-h-[82svh] sm:px-10 sm:pb-24 lg:min-h-[88vh] lg:h-full lg:px-14 lg:pb-28">
        <div className="max-w-2xl">
          ...
          <div className="mt-6 grid grid-cols-2 gap-2 sm:mt-7 sm:flex sm:flex-wrap sm:gap-3">
            <Button asChild size="lg" className="h-11 w-full bg-white px-4 text-sm font-bold sm:h-12 sm:w-auto sm:px-6 sm:text-base text-black hover:bg-white/85">{playableContentId ? <Link to="/watch/$contentId" params={{ contentId: playableContentId }}><Play className="fill-current" />Play</Link> : item.available ? <Link to="/checkout/$productId" params={{ productId: BACKEND_PRODUCT_IDS.allAccess }} search={{returnTo:`/title/${item.slug}`,origin:"/",originScroll:String(typeof window!=="undefined"?window.scrollY:0)}} onClick={()=>rememberReturnContext("home")}><Play className="fill-current" />Get Access</Link> : <Link to="/title/$slug" params={{ slug: item.slug }} onClick={()=>rememberReturnContext("home")}><Play className="fill-current" />More Info</Link>}</Button>
            <Button asChild size="lg" variant="secondary" className="h-11 w-full bg-white/20 px-4 text-sm font-bold sm:h-12 sm:w-auto sm:px-6 sm:text-base text-white backdrop-blur-md hover:bg-white/30"><Link to="/title/$slug" params={{ slug: item.slug }} onClick={()=>rememberReturnContext("home")}><Info />More Info</Link></Button>
