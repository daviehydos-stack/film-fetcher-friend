...
export function SiteHeader(){
 const[open,setOpen]=useState(false),[scrolled,setScrolled]=useState(false),[admin,setAdmin]=useState(false),[customer,setCustomer]=useState<any>(null),[authReady,setAuthReady]=useState(false),[authBusy,setAuthBusy]=useState(false);
 useEffect(()=>{let live=true;void customerSession().then(user=>{if(!live)return;setCustomer(user);setAdmin(String(user?.email||"").toLowerCase()===ADMIN_EMAIL);setAuthReady(true)}).catch(()=>{if(live)setAuthReady(true)});return()=>{live=false}},[]);
